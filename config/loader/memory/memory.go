...
import (
	...
	"github.com/lack-io/vine/config/loader"
	"github.com/lack-io/vine/config/reader"
	"github.com/lack-io/vine/config/reader/json"
...
func (m *memory) Get(path ...string) (reader.Value, error) {
	...
	// we are truly crewed, trying to load in a hacked way
...
func (m *memory) Load(sources ...source.Source) error {
	...
	for _, item := range sources {
		set, err := item.Read()
		if err != nil {
			gerrors = append(gerrors, fmt.Sprintf("error loading source %s: %v", item, err))
			// continue processing
			continue
		}
		m.Lock()
