...
func TestEnvvar_Prefixes(t *testing.T) {
	...
	os.Setenv("VINE_REGISTRY", "mdns")

	var prefixtests = []struct {
		prefixOpts   []source.Option
		expectedKeys []string
	}{
		{[]source.Option{WithPrefix("APP", "VINE")}, []string{"app", "vine"}},
		{[]source.Option{WithPrefix("VINE"), WithStrippedPrefix("APP")}, []string{"database", "vine"}},
		{[]source.Option{WithPrefix("VINE"), WithStrippedPrefix("APP")}, []string{"database", "vine"}},
	}
...
func TestEnvvar_WatchNextNoOpsUntilStop(t *testing.T) {
	src := NewSource(WithStrippedPrefix("GOVINE_"))
	w, err := src.Watch()
	if err != nil {
		t.Error(err)
	}
