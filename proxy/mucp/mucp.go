...
import (
	...
	"github.com/lack-io/vine/codec/bytes"
...
func (p *Proxy) filterRoutes(ctx context.Context, routes []router.Route) []router.Route {
	...
	log.Debugf("Proxy filtered routes %+v", filteredRoutes)
...
func (p *Proxy) getLink(r router.Route) (client.Client, error) {
	...
	l, ok := p.Links[r.Link]
	if !ok {
		return nil, errors.InternalServerError(proxy.DefaultName, "link not found")
	}
	return l, nil
}
...
func (p *Proxy) cacheRoutes(service string) ([]router.Route, error) {
	...
	if err != nil {
		...
		log.Debugf("Failed to lookup route for %s: %v", service, err)
...
func (p *Proxy) manageRoutes(route router.Route, action string) error {
	...
	log.Debugf("Proxy taking route action %v %+v\n", action, route)
...
func (p *Proxy) ProcessMessage(ctx context.Context, msg server.Message) error {
	...
	log.Debugf("Proxy received message for %s", msg.Topic())
...
func (p *Proxy) ServeRequest(ctx context.Context, req server.Request, rsp server.Response) error {
	...
	if len(service) == 0 {
		return errors.BadRequest(proxy.DefaultName, "service name is blank")
	}

	log.Debugf("Proxy received request for %s %s", service, endpoint)
	...
	if len(routes) == 0 {
		return errors.InternalServerError(proxy.DefaultName, "route not found")
	}
