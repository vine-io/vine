...
type UserIndex struct {
	Name         string `dao:"index"`
	Name2        string `dao:"index:idx_name,unique"`
	Name3        string `dao:"index:,sort:desc,collate:utf8,type:btree,length:10,where:name3 != 'jinzhu'"`
	Name4        string `dao:"uniqueIndex"`
	Name5        int64  `dao:"index:,class:FULLTEXT,comment:hello \\, world,where:age > 10"`
	Name6        int64  `dao:"index:profile,comment:hello \\, world,where:age > 10"`
	Age          int64  `dao:"index:profile,expression:ABS(age),option:WITH PARSER parser_name"`
	OID          int64  `dao:"index:idx_id;index:idx_oid,unique"`
	MemberNumber string `dao:"index:idx_id,priority:1"`
}
