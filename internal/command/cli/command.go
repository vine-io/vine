...
import (
	...
	inclient "github.com/lack-io/vine/internal/client"
...
func Publish(c *cli.Context, args []string) error {
	if len(args) < 2 {
		return errors.New("require topic and message e.g vine publish event '{\"hello\": \"world\"}'")
	}
	defer func() {
...
func CallService(c *cli.Context, args []string) ([]byte, error) {
	if len(args) < 2 {
		return nil, errors.New(`require service and endpoint e.g vine call greeeter Say.Hello '{"name": "john"}'`)
	}
