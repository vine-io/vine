...
func (t *tun) sendTo(links []*link, msg *message) error {
	...
	for _, link := range links {
		...
		logger.Debugf("Tunnel sending %+v to %s", newMsg.Header, link.Remote())
...
func (t *tun) listen(link *link) {
	...
	for {
		...
		case "session":
			// process message
			logger.Debugf("Tunnel received %+v from %s", msg.Header, link.Remote())
...
func (t *tun) Dial(channel string, opts ...tunnel.DialOption) (tunnel.Session, error) {
	...
	if len(options.Link) > 0 {
		...
		if len(links) == 0 {
			...
			logger.Debugf("Tunnel deleting session %s %s: %v", c.session, c.channel, tunnel.ErrLinkNotFound)
