...
func (t *tunListener) process() {
	...
	for {
		...
		case m := <-t.session.recv:
			...
			logger.Tracef("Tunnel listener received channel %s session %s type %s exists: %t", m.channel, m.session, m.typ, ok)
			if !ok {
			...
			case <-sess.closed:
				delete(conns, sessionId)
			case sess.recv <- m:
				logger.Tracef("Tunnel listener sent to recv chan channel %s session %s type %s", m.channel, sessionId, m.typ)
			}
		}
	}
}
