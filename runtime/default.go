...
import (
	...
	"github.com/lack-io/vine/runtime/local/git"
...
type runtime struct {
	...
	running bool
	// namespaces stores services grouped by namespace, e.g. namespaces["foo"]["go.vine.auth:latest"]
	// would return the latest version of go.vine.auth from the foo namespace
...
func NewRuntime(opts ...Option) Runtime {
	...
	path := filepath.Join(os.TempDir(), "vine", "logs")
...
func (r *runtime) checkoutSourceIfNeeded(s *Service) error {
	...
	cpath := filepath.Join(os.TempDir(), "vine", "uploads", s.Source)
...
func (r *runtime) run(events <-chan Event) {
	...
	processEvent := func(event Event, service *service, ns string) error {
		...
		log.Debugf("Runtime updating service %s in %v namespace", name, ns)
			...
			case Update:
				if event.Service != nil {
					...
					if _, ok := r.namespaces[ns]; !ok {
						log.Debugf("Runtime unknown namespace: %s", ns)
						r.RUnlock()
						continue
					}
...
func (l *logStream) Stop() error {
	...
	default:
		...
		err := l.tail.Stop()
		if err != nil {
			log.Errorf("Error stopping tail: %v", err)
			return err
		}
	}
	return nil
...
func (r *runtime) Update(s *Service, opts ...UpdateOption) error {
	...
	if err := service.Stop(); err != nil && err.Error() != "no such process" {
		log.Errorf("Error stopping service %s: %s", service.Name, err)
		return err
	}
