...
Plugins are a way of integrating external code into the Vine toolkit. This is completely separate to go-vine plugins. 
Using plugins here allows you to add additional flags, commands and HTTP handlers to the toolkit. 

## How it works

There is a global plugin manager under vine/plugin which consists of plugins that will be used across the entire toolkit. 
...
func init() {
	plugin.Register(plugin.NewPlugin(
		...
		plugin.WithInit(func(ctx *cli.Context) error {
			log.Println("Got value for example-flag", ctx.String("example-flag"))
			return nil
		}),
	))
}
...
Simply build vine with the plugin

```shell
go build -o vine ./main.go ./plugin.go
```
