...
import (
	...
	"github.com/lack-io/vine/api/router/util"
	"github.com/lack-io/vine/log"
	"github.com/lack-io/vine/registry"
	"github.com/lack-io/vine/registry/cache"
...
func (r *registryRouter) refresh() {
	...
	for {
		...
		for _, s := range services {
			...
			if err != nil {
				log.Errorf("unable to get service: %v", err)
				continue
			}
			r.store(service)
		}
...
func (r *registryRouter) store(services []*registry.Service) {
	...
	for _, service := range services {
		...
		for _, sep := range service.Endpoints {
			...
			if err := api.Validate(end); err != nil {
				log.Debugf("endpoint validation failed: %+v", err)
				continue
			}
	...
	for name, ep := range eps {
		...
		for _, h := range ep.Endpoint.Host {
			...
			if err != nil {
				log.Debugf("endpoint have invalid host regexp: %+v", err)
				continue
			}
		...
		for _, p := range ep.Endpoint.Path {
			...
			if err != nil && !pcreok {
				log.Debugf("endpoint have invalid path pattern: %+v", err)
				continue
...
func (r *registryRouter) watch() {
	...
	for {
		...
		for {
			...
			res, err := w.Next()
			if err != nil {
				log.Errorf("error getting next endpoint: %+v", err)
				close(ch)
				break
			}
			r.process(res)
...
func (r *registryRouter) Endpoint(req *http.Request) (*api.Service, error) {
	...
	for n, e := range r.eps {
		...
		log.Debugf("api method match %s", req.Method)
		...
		log.Debugf("api host match %s", req.URL.Host)

		// 3. try path via google.api path matching
		for _, pathreg := range cep.pathregs {
			matches, err := pathreg.Match(path, "")
			if err != nil {
				log.Debugf("api gpath not match %s != %v", path, pathreg)
				continue
			}
			log.Debugf("api gpath match %s = %v", path, pathreg)
			pMatch = true
			ctx := req.Context()
		...
		if !pMatch {
			...
			for _, pathreg := range cep.pcreregs {
				if !pathreg.MatchString(req.URL.Path) {
					log.Debugf("api pcre path not match %s != %v", path, pathreg)
					continue
				}
				log.Debugf("api pcre path match %s != %v", path, pathreg)
				pMatch = true
				break
			}
		}
