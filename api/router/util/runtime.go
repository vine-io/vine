...
func NewPattern(version int, ops []int, pool []string, verb string, opts ...PatternOpt) (Pattern, error) {
	...
	if version != 1 {
		log.Debugf("unsupported version: %d", version)
		return Pattern{}, ErrInvalidPattern
	}

	l := len(ops)
	if l%2 != 0 {
		log.Debugf("odd number of ops codes: %d", l)
		return Pattern{}, ErrInvalidPattern
	}
	...
	for i := 0; i < l; i += 2 {
		...
		case OpLitPush:
			if op.operand < 0 || len(pool) <= op.operand {
				log.Debugf("negative literal index: %d", op.operand)
				return Pattern{}, ErrInvalidPattern
			}
			if pushMSeen {
				tailLen++
		...
		case OpConcatN:
			if op.operand <= 0 {
				log.Debugf("negative concat size: %d", op.operand)
				return Pattern{}, ErrInvalidPattern
			}
			stack -= op.operand
			if stack < 0 {
		...
		case OpCapture:
			if op.operand < 0 || len(pool) <= op.operand {
				log.Debugf("variable name index out of bound: %d", op.operand)
				return Pattern{}, ErrInvalidPattern
			}
			v := pool[op.operand]
			op.operand = len(vars)
		...
		default:
			log.Debugf("invalid opcode: %d", op.code)
			return Pattern{}, ErrInvalidPattern
		}
...
func MustPattern(p Pattern, err error) Pattern {
	if err != nil {
		log.Fatalf("Pattern initialization failed: %v", err)
	}
	return p
}
