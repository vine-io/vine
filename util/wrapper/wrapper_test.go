...
func TestAuthHandler(t *testing.T) {
	...
	debugReq := testRequest{service: "go.vine.service.foo", endpoint: "Debug.Foo"}
	serviceReq := testRequest{service: "go.vine.service.foo", endpoint: "Foo.Bar"}
	...
	t.Run("BlankNamespaceHeader", func(t *testing.T) {
		...
		if ns, _ := metadata.Get(inCtx, "Vine-Namespace"); ns != a.namespace {
	...
	t.Run("ValidNamespaceHeader", func(t *testing.T) {
		...
		inNs := "reqnamespace"
		inCtx := metadata.Set(context.TODO(), "Vine-Namespace", inNs)
		h := func(ctx context.Context, req server.Request, rsp interface{}) error {
			inCtx = ctx
			return nil
		}
		...
		if ns, _ := metadata.Get(inCtx, "Vine-Namespace"); ns != inNs {
...
func TestCacheWrapper(t *testing.T) {
	req := client.NewRequest("go.vine.service.foo", "Foo.Bar", nil)
