...
func (w *workersKV) request(ctx context.Context, method, path string, body interface{}, headers http.Header) ([]byte, http.Header, int, error) {
	...
	if body != nil {
		if paramBytes, ok := body.([]byte); ok {
			...
			if err != nil {
				return nil, nil, 0, fmt.Errorf("%w error marshalling params to JSON", err)
			}
		}
	} else {
		jsonBody = nil
	...
	if err != nil {
		return nil, nil, 0, fmt.Errorf("%w error creating new request", err)
	}
	...
	// set the user agent to vine
	req.Header.Set("User-Agent", "vine/1.0 (https://vine.mu)")
