...
func TestParse(t *testing.T) {
	entries := []struct {
		...
		{
			expr:     "@lastDay 5m",
			expected: ConstantLastCron{Delay: time.Duration(5) * time.Minute},
		},
		{
			expr: "@lastDay Xm",
			err:  "Failed to parse duration",
...
func TestStandardSpecCron(t *testing.T) {
	entries := []struct {
		expr     string
		expected Crontab
		err      string
		...
		{
			expr:     "* * * * * *",
			expected: &SpecCron{all(seconds), all(minutes), all(hours), all(dom), all(months), all(dow)},
		},
		{
			expr:     "@every 5m",
			expected: ConstantDelayCron{time.Duration(5) * time.Minute},
		},
		{
			expr: "* * * *",
			err:  "Expected exactly 6 fields",
		},
	}
