	...
	for {
		...
		case m := <-t.session.recv:
			...
			sess, ok := conns[sessionId]
				log.Debugf("Tunnel listener received channel %+v session %s type %s exists: %t", m.channel, m.session, m.typ, ok)
			if !ok {
			...
			case <-sess.closed:
				delete(conns, sessionId)
			case sess.recv <- m:
					log.Debugf("Tunnel listener sent to recv chan channel %+v session %s type %s", m.channel, sessionId, m.typ)
			}
		}
	}
}
