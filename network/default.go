...
import (
	...
	rtr "github.com/lack-io/vine/client/selector/router"
	"github.com/lack-io/vine/log"
	"github.com/lack-io/vine/network/resolver/dns"
	pbNet "github.com/lack-io/vine/network/service/proto"
	"github.com/lack-io/vine/proxy"
	"github.com/lack-io/vine/router"
	pbRtr "github.com/lack-io/vine/router/service/proto"
	...
	bun "github.com/lack-io/vine/tunnel/broker"
	tun "github.com/lack-io/vine/tunnel/transport"
...
func (n *network) initNodes(startup bool) {
	...
	log.Debugf("Network initialising nodes %+v\n", init)
	n.tunnel.Init(
		tunnel.Nodes(nodes...),
	)
}
...
func (n *network) getRouteMetric(router string, gateway string, link string) int64 {
	...
	log.Debugf("Network looking up %+v link to gateway: %s", link, gateway)
...
func (n *network) processCtrlChan(listener tunnel.Listener) {
	...
	for {
		...
		case m := <-recv:
			...
			switch m.msg.Header["Vine-Method"] {
			case "advert":
				...
				for _, event := range pbRtrAdvert.Events {
					...
					log.Debugf("Network metric for router %+v and gateway %s: %v", event.Route.Router, event.Route.Gateway, metric)
				...
				if len(events) == 0 {
					log.Debugf("Network no events to be processed by router: %+v", n.options.Id)
					continue
				}
				...
				log.Debugf("Network router %+v processing advert: %s", n.Id(), advert.Id)
		...
		case m := <-recv:
			...
			case "connect":
				...
				if err := n.AddPeer(peer); err == ErrPeerExists {
					log.Debugf("Network peer exists, refreshing: %+v", peer.id)
			...
			case "peer":
				...
				log.Debugf("Network peer exists, refreshing: %+v", pbNetPeer.Node.Id)
				...
				// update the link
				peer.link = m.msg.Header["Vine-Link"]

				log.Debugf("Network updating topology of node: %+v", n.node.id)
...
func (n *network) updatePeerLinks(peer *node) error {
	...
	log.Debugf("Network looking up link %+v in the peer links", linkId)
	...
	log.Debugf("Network updating peer links for peer %+v", peer.address)
