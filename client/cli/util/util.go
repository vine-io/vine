...
import (
	...
	merrors "github.com/lack-io/vine/service/errors"
)

const (
	// EnvLocal is a builtin environment, it represents your local `vine server`
	EnvLocal = "local"
...
var defaultEnvs = map[string]Env{
	EnvLocal: {
		...
		Description:  "Local running Vine Server",
	},
	EnvDev: {
		Name:         EnvDev,
	...
	EnvPlatform: {
		...
		Description:  "Cloud hosted Vine Platform",
	},
}
...
func CLIProxyAddress(ctx *cli.Context) (string, error) {
	...
	// fix for "vine service [command]", e.g "vine service auth"
...
func GetEnvByName(env string) (Env, error) {
	...
	envir, ok := envs[env]
	if !ok {
		return Env{}, fmt.Errorf("Env \"%s\" not found. See `vine env` for available environments.", env)
	}
	return envir, nil
}
...
func CliError(err error) cli.ExitCoder {
	...
	if address := regexp.MustCompile(`Error while dialing dial tcp.*?([\w]+\.[\w:\.]+): `).FindStringSubmatch(err.Error()); len(address) > 0 {
		return cli.Exit(fmt.Sprintf(`Failed to connect to vine server at %s`, address[1]), 4)
	}
