...
import (
	...
	run "github.com/lack-io/vine/internal/runtime"
...
// ie vine update invite becomes vine update github.com/m3o/services/invite
func appendSourceBase(ctx *cli.Context, workDir, source string, matchExistingService bool) string {
	...
	if !isLocal && err != nil {
		...
		if len(baseURL) == 0 {
			return path.Join("github.com/vine/services", source)
		}
...
func runService(ctx *cli.Context) error {
	...
	for _, evar := range ctx.StringSlice("env-vars") {
...
func getService(ctx *cli.Context) error {
	...
	for _, service := range services {
		...
		// argument passed to vine run is always kept in the source attribute of service metadata
