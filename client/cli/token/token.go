...
// tToken files consist of one line per token.
package token
...
func tokenKey(ctx *cli.Context, accountID string) (string, error) {
	...
	return fmt.Sprintf("vine://%v/%v/%v", env.ProxyAddress, ns, accountID), nil
}
...
func saveToUserConfig(envName string, token *auth.AccountToken) error {
	if err := config.Set(config.Path("vine", "auth", envName, "token"), token.AccessToken); err != nil {
		return err
	}
	// Store the refresh token in vine config
	if err := config.Set(config.Path("vine", "auth", envName, "refresh-token"), token.RefreshToken); err != nil {
		return err
	}
	// Store the refresh token in vine config
	return config.Set(config.Path("vine", "auth", envName, "expiry"), fmt.Sprintf("%v", token.Expiry.Unix()))
}
...
func removeFromUserConfig(envName string) error {
	if err := config.Set(config.Path("vine", "auth", envName, "token"), ""); err != nil {
		return err
	}
	// Store the refresh token in vine config
	if err := config.Set(config.Path("vine", "auth", envName, "refresh-token"), ""); err != nil {
		return err
	}
	// Store the refresh token in vine config
	return config.Set(config.Path("vine", "auth", envName, "expiry"), "")
}
