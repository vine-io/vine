...
func (s *service) Init(opts ...Option) error {
	...
	serviceOpts := []vine.Option{}

	if len(s.opts.Flags) > 0 {
		serviceOpts = append(serviceOpts, vine.Flags(s.opts.Flags...))
	}

	if s.opts.Registry != nil {
		serviceOpts = append(serviceOpts, vine.Registry(s.opts.Registry))
	}

	s.Unlock()

	serviceOpts = append(serviceOpts, vine.Action(func(ctx *cli.Context) error {
		s.Lock()
		defer s.Unlock()
	...
	if s.opts.Service.Name() == "" {
		serviceOpts = append(serviceOpts, vine.Name(s.opts.Name))
	}
	serviceOpts = append(serviceOpts, vine.Version(s.opts.Version))
	s.RUnlock()
