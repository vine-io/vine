...
func (g *binaryGitter) checkoutAnyRemote(repo, branchOrCommit string, useCredentials bool) error {
	...
	// Assumes remote address format is git@gitlab.com:vine-test/monorepo-test.git
...
func (g *binaryGitter) checkoutGitLabPublic(repo, branchOrCommit string) error {
	// Example: https://gitlab.com/vine-test/basic-vine-service/-/archive/master/basic-vine-service-master.tar.gz
	// @todo if it's a commit it must not be checked out all the time
	repoFolder := strings.ReplaceAll(strings.ReplaceAll(repo, "/", "-"), "https:--", "")
	g.folder = filepath.Join(os.TempDir(),
		repoFolder+"-"+shortid.MustGenerate())
	...
	// It has the format of eg. basic-vine-service-master-314b4a494ed472793e0a8bce8babbc69359aed7b
...
func (s *Source) RuntimeName() string {
	if len(s.Folder) == 0 {
		// This is the case for top level url source ie. gitlab.com/vine-test/basic-vine-service
...
var nameExtractRegexp = regexp.MustCompile(`((vine|web)\.Name\(")(.*)("\))`)
