...
func (m *manager) createServiceInRuntime(srv *service) error {
	...
	if len(acc.ID) > 0 && len(acc.Secret) > 0 {
		options = append(options, gorun.WithSecret("VINE_AUTH_ID", acc.ID))
		options = append(options, gorun.WithSecret("VINE_AUTH_SECRET", acc.Secret))
	}
...
func (m *manager) runtimeEnv(srv *gorun.Service, options *gorun.CreateOptions) []string {
	...
	env := map[string]string{
		...
		"VINE_SERVICE_NAME":    srv.Name,
		"VINE_SERVICE_VERSION": srv.Version,
		// set the proxy for the service to use (e.g. vine network)
		// using the proxy which has been configured for the runtime
		"VINE_PROXY": client.DefaultClient.Options().Proxy,
	}

	// bind to port 8080, this is what the k8s tcp readiness check will use
	if runtime.DefaultRuntime.String() == "kubernetes" {
		env["VINE_SERVICE_ADDRESS"] = ":8080"
	}
	...
	if len(options.Namespace) > 0 {
		env["VINE_NAMESPACE"] = options.Namespace
	}
