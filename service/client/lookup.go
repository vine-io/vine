...
func LookupRoute(ctx context.Context, req Request, opts CallOptions) ([]string, error) {
	...
	if err == router.ErrRouteNotFound {
		return nil, errors.InternalServerError("go.vine.client", "service %s: %s", req.Service(), err.Error())
	} else if err != nil {
		return nil, errors.InternalServerError("go.vine.client", "error getting next %s node: %s", req.Service(), err.Error())
	}
