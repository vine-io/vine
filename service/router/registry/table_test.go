...
func TestQuery(t *testing.T) {
	...
	routes, err = table.Query(router.QueryRouter(_router))
	if err != nil {
		t.Errorf("error looking up routes: %s", err)
	}
	...
	if routes[0].Router != _router {
		t.Errorf("incorrect route returned. Expected router: %s, found: %s", _router, routes[0].Router)
	}
