...
func (s *sqlStore) initDB(database, table string) error {
	...
	if err != nil {
		return fmt.Errorf("%w Couldn't set database", err)
	}
	...
	if err != nil {
		return fmt.Errorf("%w Couldn't create table", err)
	}
...
func (s *sqlStore) read(key string, options store.ReadOptions) ([]*store.Record, error) {
	...
	if err != nil {
		...
		return []*store.Record{}, fmt.Errorf("%w sqlStore.read failed", err)
	}
...
func (s *sqlStore) Write(r *store.Record, opts ...store.WriteOption) error {
	...
	if err != nil {
		return fmt.Errorf("%w Couldn't insert record "+r.Key, err)
	}
...
// NewStore returns a new vine Store backed by sql
func NewStore(opts ...store.Option) store.Store {
	...
	if err := s.configure(); err != nil {
		logger.Errorf("Error configuring store %v", err)
	}
