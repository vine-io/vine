...
// Driver is a dialect.Driver implementation of SQL based databases.
type Driver struct {
	Conn
	dialect string
}
...
func (d Driver) Dialect() string {
	// If the underlying driver is wrapped with opencensus dirver
...
// ExecQuerier wrapps the standard Exec and Query methods.
type ExecQuerier interface {
	ExecContext(ctx context.Context, query string, args ...interface{}) (sql.Result, error)
	QueryContext(ctx context.Context, query string, args ...interface{}) (*sql.Rows, error)
}

// Conn implents dialect.ExecQuerier given ExecQuerier.
type Conn struct {
	ExecQuerier
}
...
type (
	...
	// NullInt64 is an alias to sql.NulInt64
