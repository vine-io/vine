...
import (
	...
	"github.com/lack-io/vine/service/ent/schema/field"
)
...
// WithForeignKeys sets the foreign-key in ddl. Defaults to true.
...
func (m *Migrate) changeSet(curr, new *Table) (*changes, error) {
	...
	if len(curr.PrimaryKey) != len(new.PrimaryKey) {
		return nil, fmt.Errorf("cannot change primary key for table: %v", curr.Name)
	}
	...
	for _, c1 := range curr.Columns {
		// If a column was dropped, multi-columns indexes that are associated with its column will
...
// fixture is a special migration code for renaming foreign-key columns
func (m *Migrate) fixture(ctx context.Context, tx dialect.Tx, curr, new *Table) error {
	...
	for _, fk := range new.ForeignKeys {
		...
		if err != nil {
			return fmt.Errorf("checking foreign-key existance %q: %v", fk.Symbol, err)
		}
		if !ok {
			continue
		}
		...
		// Find all indexes that maybe need to renamed.
		for _, idx := range prev.indexes {
			switch _, ok := new.index(idx.Name); {
			// Ignore that exist in the schema. Pks.
			case ok || idx.primary:
		...
		// Update the name of loaded column, so `changeSet` won't create it.
		prev.Name = newcol.Name
	}
	// Go over the indexes that need to be renamed
	// and find identical in the new schema.
...
// symbol makes sure the symbol length is not longer than the max length in the dialect.
...
// exists checks if the given COUNT query returns a value >= 1.
func exists(ctx context.Context, tx dialect.Tx, query string, args ...interface{}) (bool, error) {
	rows := &sql.Rows{}
	if err := tx.Query(ctx, query, args, rows); err != nil {
...
// fkRenamer is used by the fixture migration and it's
// implemented by the different dialects for renaming FKs.
