...
import (
	...
	"github.com/lack-io/vine/service/ent/dialect/entsql"
	"github.com/lack-io/vine/service/ent/dialect/sql"
	"github.com/lack-io/vine/service/ent/schema/field"
)

const (
	// DefaultStringLen describe the default length for string/varchar types.
...
func (t *Table) index(name string) (*Index, bool) {
	...
	if !ok {
		...
		name = strings.TrimPrefix(name, "_key")
		c, ok = t.column(name)
	}
	if ok && c.Unique {
...
type Column struct {
	...
	Nullable   bool              // nul or not null attribute
...
// Unique returns boolean indicates if this column is a unique key.
...
// UintType reports of the given type is a uint type (uint8 ... uint64).
...
// defaultValue adds tge `DEFAULT` attribute the column.
...
// ConstName returns the constant name of a reference option. It's used by entc for printing the constant name in template.
...
// as the given index (expect the name).
...
// multiple sql rows can represent the same index (multi-column indexes).
type Indexes []*Index
