...
func (s *rpcServer) HandleEvent(e broker.Event) error {
	...
	// Vine-Service means a request
	// Vine-Topic means a message

	rpcMsg := &rpcMessage{
		topic:       msg.Header["Vine-Topic"],
		contentType: ct,
...
func (s *rpcServer) ServeConn(sock transport.Socket) {
	...
	var gerr error
	// streams are multiplexed no Vine-Stream or Vine-Id header
	pool := socket.NewPool()
	...
	for {
		...
		// Vine-Service is a request
		// Vine-Topic is a message
		if t := msg.Header["Vine-Topic"]; len(t) > 0 {
			// process the event
			ev := newEvent(msg)
			// TODO: handle the error event
			if err := s.HandleEvent(ev); err != nil {
				msg.Header["Vine-Error"] = err.Error()
			}
			// write back some 200
		...
		// use Vine-Stream as the stream identifier
		// in the event its blank we'll always process
		// on the same socket
		id := msg.Header["Vine-Stream"]

		// if there's no stream id then its a standard request
		// use the Vine-Id
		if len(id) == 0 {
			id = msg.Header["Vine-Id"]
		}

		// check stream id
		var stream bool

		if v := getHeader("Vine-Stream", msg.Header); len(v) > 0 {
			stream = true
		}
		...
		go func(id string, psock *socket.Socket) {
			...
			if serveRequestErr := r.ServeRequest(ctx, request, response); serveRequestErr != nil {
				// write an error response
				writeError := rcodec.Write(&codec.Message{
					Header: msg.Header,
					Error:  serveRequestErr.Error(),
					Type:   codec.Error,
				}, nil)
				...
				alreadyClosed := serveRequestErr == lastStreamResponseError && writeError == io.EOF
...
func (s *rpcServer) Start() error {
	...
	if err = s.opts.RegisterCheck(s.opts.Context); err != nil {
		...
		if err = s.Register(); err != nil {
			logger.Errorf("Server %s-%s register error: %v", config.Name, config.Id, err)
		}
	}
