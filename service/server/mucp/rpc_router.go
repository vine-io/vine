...
func (router *router) ProcessMessage(ctx context.Context, msg server.Message) (err error) {
	defer func() {
		...
		if r := recover(); r != nil {
			...
			err = merrors.InternalServerError("go.vine.server", "panic recovered: %v", r)
		}
	}()
	...
	if len(errResults) > 0 {
		err = merrors.InternalServerError("go.vine.server", "subscriber error: %v", strings.Join(errResults, "\n"))
	}
