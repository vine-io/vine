...
func (router *router) ProcessMessage(ctx context.Context, msg server.Message) (err error) {
	defer func() {
		...
		if r := recover(); r != nil {
			...
			err = verrors.InternalServerError(server.DefaultName, "panic recovered: %v", r)
		}
	}()
	...
	for _, sub := range subs {
		...
		for i := 0; i < len(sub.handlers); i++ {
			...
			fn := func(ctx context.Context, msg server.Message) error {
				...
				// execute the actually call of the handler
	...
	if len(errResults) > 0 {
		err = verrors.InternalServerError(server.DefaultName, "subscriber error: %v", strings.Join(errResults, "\n"))
	}
