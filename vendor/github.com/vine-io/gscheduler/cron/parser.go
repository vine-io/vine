...
func (p Parser) Parse(spec string) (Crontab, error) {
	if len(spec) == 0 {
...
func ParseStandard(standardSpec string) (Crontab, error) {
...
//   - Descriptors, e.g. "@midnight", "@every 1h30m"
func Parse(spec string) (Crontab, error) {
...
func parseDescriptor(descriptor string) (Crontab, error) {
	switch descriptor {
	...
	if strings.HasPrefix(descriptor, lastDay) {
		duration, err := time.ParseDuration(descriptor[len(lastDay):])
		if err != nil {
			return nil, fmt.Errorf("Failed to parse duration %s: %s", descriptor, err)
		}
		return LastDay(duration), nil
	}
